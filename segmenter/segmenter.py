...
class Segmenter():
  ...
  def segment_on_silence(self, sound_file_path, destination, title, silence_threshold_in_db = 75, trim_threshold_in_db = 20):
    ...
    trimed_segments = [{**seg, **{'signal': self.trim_signal(seg['signal'], trim_threshold_in_db), 'sound_file_path': sound_file_path}} for seg in splited_segments]
  ...
  def segment_on_samples(self, title, sound_file_path, destination, start, end, file_prefix, trim_threshold_in_db = 20):
    ...
    trimed_segments = [{**seg, **{'signal': self.trim_signal(np.array(seg['signal']), trim_threshold_in_db), 'sound_file_path': sound_file_path}} for seg in segments]
  ...
  def chunks_to_signal(self, orginal_signal, frames):
    ...
    segments = [{'start': f[0].item(), 'end': f[-1].item(), 'signal': orginal_signal[f[0]:f[-1]]} for f in frames]
    return segments
  
  def split_signal(self, signal, silence_threshold=75):
    non_silence_frames = librosa.effects.split(signal, top_db=silence_threshold)
