...
def segment_on_silence(title:str, sound_file_path: str, destination: str, silence_threshold_in_db: Optional[int] = 75, trim_threshold_in_db:  Optional[int] = 20):
    meta = segmenter.segment_on_silence(sound_file_path, destination, title, silence_threshold_in_db, trim_threshold_in_db)
...
def segment_on_samples(data:SegmentOnSamples):
    meta = segmenter.segment_on_samples(data.title, data.source, data.destination, data.start, data.end, data.file_prefix, data.trim_threshold_in_db)
    annotations = [AnnotationSchema(**m) for m in meta]
    annotation_controller.add_annotations(annotations)
    return meta
...
@app.get('/annotations/title')
def get_all_title():
    return annotation_controller.get_all_title();

@app.get('/annotations/title/{title}')
