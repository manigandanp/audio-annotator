...
முதல் பாகம்.
புது வெள்ளம்.
அத்தியாயம் ஒன்று - ஆடித்திருநாள்
...
def get_segment_ids(segments_url):
    res = requests.get(segments_url);
    data = res.json();
    segment_ids = [d['id'] for d in data]; 
    return segment_ids;

def post_annotations():
    segments_url = "http://localhost:3000/api/segments"
    annotations_url = "http://localhost:3000/api/annotations"
    segment_ids = get_segment_ids(segments_url)
    sentences = transcription.split(" ")
    for id in segment_ids:
        num = round(random.random() * len(sentences) - 15)
        word_len = round(random.random() * 50)
        annotation = " ".join(sentences[num:num + word_len]).strip()
        annotation = re.sub(r"\n+|\t+|\r+", " ", annotation)
        data =  {'segmentId':id, 'annotation': annotation}
        post(annotations_url, data)
